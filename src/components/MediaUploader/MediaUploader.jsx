...
export const MediaUploader = ({
    ...
    const uploadMedia = async (e, type) => {
        ...
        if (type === "image") {
            ...
            await fetch(`${url}/video`, {
                ...
                .then((result) => {
                    if (result.generatedFilename) {
                        const mediaFile = `${url}/unsecure/video/${result.generatedFilename}`;
    ...
    const addSlide = () => {
        const cloneSlide = target.parentElement.cloneNode();
        const cloneImage = target.cloneNode();
        cloneSlide.appendChild(cloneImage);
        cloneSlide.classList.remove("swiper-slide-active");
        target.parentElement.parentElement.appendChild(cloneSlide);
    };
                ...
                <FlexWrap gap={32}>
                    <FlexWrap direction="column" gap={32}>
                        ...
                        <MediaUploaderPreviewStyled>
                            <img
                                src={
                                    image ? image : "/images/svg/image-plug.svg"
                                }
                                alt="malanka"
                            />
                        </MediaUploaderPreviewStyled>
                        ...
                        >
                            {imageType === "adv" && (
                                <Input
                                    placeholder="Введите ссылку на рекламодателя, пример: https://malankabn.by"
                                    value={advLink}
                            ...
                            {slider && (
                                <FlexWrap justify="space-between" gap={32}>
                                    ...
                                    <Button primary onClick={addSlide}>
                                        Добавить слайд
                                    </Button>
                                </FlexWrap>
                            )}
            ...
            <MediaUploaderContentStyled>
                <FlexWrap gap={32}>
                    <FlexWrap direction="column" gap={32}>
                        ...
                        >
                            <Input
                                placeholder="Введите ссылку для загрузки видео"
                                value={videoLink}
                            ...
                            <FlexWrap justify="space-between" gap={32}>
                                ...
                                <Button primary onClick={uploadVideo}>
                                    Добавить видео
                                </Button>
                            </FlexWrap>
                        </FlexWrap>
    ...
    const uploadVideo = () => {
        let videoTag = target.children[0] || target;

        if (videoLink.includes(url)) {
            if (videoTag.tagName === "VIDEO") {
