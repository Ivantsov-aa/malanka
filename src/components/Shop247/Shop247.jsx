...
export const Shop247 = ({ innerWidth }) => {
    ...
    const loadPage = () => {
        ...
        handlerPageData().getContent(103, 1)
            .then(result => {
    ...
    const stepsRegistration = language === 'RU' ? [
    ...
    const sliderInit = () => {
        ...
        const currentSlide = document.querySelector('.slider-gallery__control .current');
        const pagesCount = document.querySelector('.slider-gallery__control .pages');
        pagesCount.innerHTML = innerWidth < 500 ? swiper.el.swiper.slides.length : swiper.el.swiper.slides.length / 3;

        swiper.on('slideChange', (e) => {
            currentSlide.innerHTML = innerWidth < 500 ? e.activeIndex + 1 : (e.activeIndex / 3) + 1;
        })
    }

    return (
        <main className='shop247' data-id='103' data-count='1'>
            ...
            <div className='flex column gap-64'>
                <div className='flex column'>
                    ...
                    <div className='flex adaptive'>
                        <div className='flex gap-16 column'>
                            <div className='opacity-block dark padding'>
                                <img src='/images/news/support.png' alt='shop247' />
                            ...
                            <div className='opacity-block dark padding'>
                                ...
                                <p className='regular-text medium'>
                                    {language === 'RU' ?
                                        <>
                                            <span>
                                                Все очень просто: бери товар и выходи!
                                            </span>
                                            Сканируй qr-код на входе с помощью мобильного приложения Malanka Shop, бери необходимый товар и просто выходи из магазина. Чек будет готов через пару минут
                                        </>
                                        :
                                        (language === 'EN' ?
                                            <>
                                                <span>
                                                    It's so easy: take the goods and go out!
                                                </span>
                                                Scan the QR code at the entrance using the Malanka Shop mobile application, take the necessary goods and just leave the store. The check will be ready in a couple of minutes
                                            </>
                                            :
                                            <>
                                                <span>
                                                    Усё вельмі проста: бяры тавар і выходзь!
                                                </span>
                                                Скануй qr-код на ўваходзе з дапамогай мабільнага прыкладання Malanka Shop, бяры неабходны тавар і проста выходзь з крамы. Чэк будзе гатовы праз пару хвілін
                                            </>
                                        )
                                    }
                                </p>
                        ...
                        <div className='flex gap-16 wrap'>
                            {stepsRegistration.map((step, i) => (
                                <div className='opacity-block-step mw-2 dark' key={i}>
                                    <p className="count">
                                        {i + 1}
                                    </p>
                                    <p className='regular-text medium'>
                    ...
                    <div className='flex gap-16 wrap'>
                        {rules.map((rule, i) => (
                            <div className='opacity-block mw-adaptive padding dark' key={i}>
                                <p className='regular-text medium'>
                ...
                <VideoBlock title={`${language === 'RU' ? 'Видеообзор' : (language === 'EN' ? 'Video review' : 'Відэаагляд')} Malanka Shop 247`} video='https://www.youtube.com/embed/2j5vdIF2tDg' />
                <SliderGallery innerWidth={innerWidth} title={`${language === 'RU' ? 'Заголовок' : (language === 'EN' ? 'Title' : 'Загаловак')} Malanka Shop 247`} photos={photos} />
            </div>
        </main>
    )
}