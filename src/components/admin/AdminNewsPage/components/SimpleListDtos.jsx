...
export const SimpleListDtos = ({item, i, templateRows, setTemplateRows, handleReplaceRows, handleDelete}) => {
    return (
                                    ...
                                    onClick={() => {
                                        const tempArray = templateRows.map((item, j) => (
                                            i === j ?
                                                {
                                                    ...item,
                                                    simpleRows: [...item.simpleRows, item.simpleRows[item.simpleRows.length - 1]]
                                                }
                                                :
                                                {...item}
                                        ))
                                    ...
                                    onClick={() => {
                                        const tempArray = templateRows.map((item, j) => (
                                            i === j ?
                                                {
                                                    ...item,
                                                    simpleRows: item.simpleRows.filter((el, z) => z !== k)
                                                }
                                                :
                                                {...item}
                                        ))
                                ...
                                onChange={(e) => {
                                    const tempArray = templateRows.map((item, j) => (
                                        i === j ?
                                            {
                                                ...item,
                                                simpleRows: item.simpleRows.map((el, z) => (
                                                    z === k ? e.target.value : el
                                                ))
                                            }
                                            :
                                            {...item}
